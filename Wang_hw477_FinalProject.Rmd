...
Clinton_Map <- full_ForMap1 %>%
  dplyr::select(state,`Cases.1M.pop(log)`,`Deaths.1M.pop(log)`,Poli_standing,`GDP_perCapita(log)`) %>%
...
ggplot() +
  ...
  annotate("text", x = 12.25, y = 7, label = "Clinton", size=8, colour="blue") +
  annotate("text", x = 12.25, y = 3, label = "Trump", size=8, colour="red")
...
## Population Density, GDP, COVID-19

```{r Population Density, GDP, COVID-19, echo = FALSE, warning=FALSE}
density_ForMap <- full %>%
...
ggplot(density_ForMap, aes(Density, Cases.1M.pop)) +
  geom_point() +
  geom_smooth() +
  ggtitle("COVID-19 Case & Population Density")

ggplot(density_ForMap, aes(GDP_perCapita, Cases.1M.pop)) +
  geom_point() +
  geom_smooth() +
  ggtitle("COVID-19 Case & GDP_perCapita")
```